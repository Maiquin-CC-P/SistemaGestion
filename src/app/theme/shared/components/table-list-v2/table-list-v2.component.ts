...
@Component({
  selector: 'app-table-list-v2',
  standalone: true,
  imports: [SharedModule, DynamicControlComponent],
  templateUrl: './table-list-v2.component.html',
  styleUrl: './table-list-v2.component.scss'
})
export class TableListV2Component<T> implements OnInit{
  @Input() data!: T[];
  ...
  @Input() elemntHtml: {show: boolean, nameRow: (keyof T)[]} = {show: false, nameRow: []};
  ...
  ngOnInit(): void {
    if (this.flagsDefautActions.edit) {
      this.defaultAction.push(
        this.utilsService.onGenerateItemTableAction('Editar', 'primary', 'feather icon-edit', AccionTable.EDIT, false, 'btn-0', 'btn-0')
      );
    }
    if (this.flagsDefautActions.delete) {
      this.defaultAction.push(
        this.utilsService.onGenerateItemTableAction('Eliminar', 'danger', 'feather icon-trash', AccionTable.DELETE, false, 'btn-1', 'btn-1')
      );
    }
